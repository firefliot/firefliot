...
# Onready Vars ............................
onready var DataTooltip = $CanvasLayer/DataTooltip
...
var LegendElement: PackedScene = preload("res://addons/easy_charts/utilities/containers/legend/function_legend.tscn")
...
func plot_from_dataframe(dataframe : DataFrame) -> void:
    ...
    data = dataframe.dataset.duplicate(true)
    if data.empty():
        ECUtilities._print_message("Can't plot a chart with an empty Array.",1)
        return
...
func show_slice_data(slice : Slice):
    DataTooltip.update_slice_datas(slice)
    DataTooltip.show()
